Shader "AA1/PBR_Moss"
{
    Properties
    {
        ...
        _AlbedoTopMap("Albedo Map", 2D) = "white" {}
        ...
        SubShader
        {
            ...
            Pass
            {
                ...
                sampler2D _AlbedoTopMap;
                float4 _AlbedoTopMap_ST;
                ...
                float4 frag(v2f i) : SV_TARGET
                {
                    ...
                    float3 albedoTop = tex2D(_AlbedoTopMap, TRANSFORM_TEX(i.uv, _AlbedoTopMap)).rgb;
                    
                    float2 uv_front = TRANSFORM_TEX(i.worldPos.yz, _AlbedoMap);
                    float2 uv_top = TRANSFORM_TEX(i.worldPos.xz, _AlbedoTopMap);
                    float2 uv_side = TRANSFORM_TEX(i.worldPos.xy, _AlbedoMap);
                    
                    //read texture at uv position of 3 projections
                    fixed4 color_front = tex2D(_AlbedoMap, uv_front);
                    fixed4 color_top = tex2D(_AlbedoTopMap, uv_top);
                    fixed4 color_side = tex2D(_AlbedoMap, uv_side);
                    ...
                    if (i.normal.y < 0.0)
                    {
                        uv_top = TRANSFORM_TEX(i.worldPos.xz, _AlbedoMap);
                        color_top = tex2D(_AlbedoMap, uv_top);
                    }
                    ...
                    i.col =  color_front + color_top + color_side;
                    i.col.rgb = albedo * i.col.rgb * _Exposure;
                    ...
                    normalDirection = lerp(normalDirection, worldNormal, _NormalStrength * (1 - weight.y));
